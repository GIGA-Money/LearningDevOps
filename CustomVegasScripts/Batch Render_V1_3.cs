...
// Change Log (V1.3) Prototyping the Reder regions in selection feature.
using System;
using System.IO;
using System.Text;
...
public class EntryPoint
{
    ...
    void DoBatchRender(ArrayList selectedTemplates, String basePath, RenderMode renderMode)
    {
        ...
        double startTimeInMilliseconds = selectionStart.ToMilliseconds();
        double endTimeInMilliseconds = (selectionStart + selectionLength).ToMilliseconds();
        ...
        foreach (RenderItem renderItem in selectedTemplates)
        {
            ...
            switch (renderMode)
            {
                ...
                case RenderMode.SelectedRegions:
                    foreach (ScriptPortal.Vegas.Region region in myVegas.Project.Regions)
                    {
                        // Extract filename without extension
                        String strippedFilename = Path.GetFileNameWithoutExtension(filename);
                        // Check for overlap if "Selected Regions" mode
                        bool overlaps = (region.Position.ToMilliseconds() >= startTimeInMilliseconds && region.Position.ToMilliseconds() < endTimeInMilliseconds) ||
                        (region.Position.ToMilliseconds() + region.Length.ToMilliseconds() > startTimeInMilliseconds && region.Position.ToMilliseconds() + region.Length.ToMilliseconds() <= endTimeInMilliseconds);
    ...
    RadioButton RenderProjectButton, RenderRegionsButton, RenderSelectionButton, RenderRegionsInSelection;
