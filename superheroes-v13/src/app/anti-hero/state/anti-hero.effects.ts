  ...
  modifyAntiHero$ = createEffect(
    () => {
      return this.actions$.pipe(
        ...
        mergeMap((data: { type: string; payload: AntiHero }) =>
          this.antiHeroService
            .updateAntiHero(data.payload.id, data.payload)
            .pipe(
              map((antiHeroes) => ({
    ...
    () => {
      return this.actions$.pipe(
        ...
        mergeMap((data: { type: string; payload: string }) =>
          this.antiHeroService.deleteAntiHero(data.payload).pipe(
            map(() => ({
    ...
    () => {
      return this.actions$.pipe(
        ...
        mergeMap((data: { type: string; payload: string[] }) =>
          forkJoin([
            ...data.payload.map((id) =>
              this.antiHeroService.deleteAntiHero(id)
            ),
          ]).pipe(
